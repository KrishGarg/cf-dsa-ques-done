...
void solve() {
  ...
  // dp[i][j] : number of distinct vertices in subtree of i at distance j from i
  vvll dp(n + 1, vll(k + 1, 0));
  fu(i, 1, n) { dp[i][0] = 1; }

  ll ans = 0;

  auto dfs = [&](ll u, ll p, auto&& f) -> void {
    for (ll v : adj[u]) {
      if (v == p) continue;
      f(v, u, f);

      fu(j, 0, k - 1) { ans += dp[u][k - (j + 1)] * dp[v][j]; }
      fu(j, 0, k - 1) { dp[u][j + 1] += dp[v][j]; }
    }
  };
