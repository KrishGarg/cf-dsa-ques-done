...
void solve() {
  ...
  // dp1[i] : number of ways to color the subtree of i, i is colored black
  // dp2[i] : number of ways to color the subtree of i, i is colored white

  // dp1[i] = children of i: j -> product(dp2[j])
  // dp2[i] = children of i: j -> product(dp1[j] + dp2[j])
  // base case: leaves: dp1[leaf] = dp2[leaf] = 1

  ll MOD = 1e9 + 7;
  vll dp1(n + 1, 1), dp2(n + 1, 1);
  ...
  auto dfs = [&](ll v, ll p, auto&& f) -> void {
    for (ll to : adj[v]) {
      ...
      dp2[v] = (dp2[v] * ((dp1[to] + dp2[to]) % MOD)) % MOD;
    }
  };

  dfs(1, 0, dfs);

  // final answer: dp1[1] + dp2[1]
  ll ans = (dp1[1] + dp2[1]) % MOD;
  cout << ans << endl;
}
