...
void solve() {
  ...
  while (!q.empty()) {
    ...
    for (auto& edge : adj[u]) {
      ll v = edge.to;
      if (edge.undI != -1) {
        if (!visitedMin[v]) minn[edge.undI] = (edge.ogUtoV ? '-' : '+');
      } else {
        if (!visitedMin[v]) {
          visitedMin[v] = 1;
          minRech++;
  ...
  cout << maxRech << endl;
  for (ch c : maxx) cout << c;
  cout << endl;

  cout << minRech << endl;
  for (ch c : minn) cout << c;
  cout << endl;
}
