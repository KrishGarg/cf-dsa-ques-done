...
struct SegTree {  // maxTree
  ...
  ll findFirst(ll v, ll x, ll lx, ll rx) {
    if (maxs[x] < v) return -1;
    if (rx - lx == 1) return lx;

    ll m = lx + (rx - lx) / 2;
    ll res = findFirst(v, left(x), lx, m);
    if (res == -1) {
      res = findFirst(v, right(x), m, rx);
    }
    return res;
  }

  ll findFirst(ll v) { return findFirst(v, 0, 0, sz); }
};
