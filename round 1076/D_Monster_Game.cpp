...
void solve() {
  ...
  vll prefStrikes(n + 1);
  fu(i, 1, n) { prefStrikes[i] = prefStrikes[i - 1] + b[i]; }

  ll score = -1;

  sort(all(a));
  fd(i, n, 1) {
    ll swordsIcanuse = n - i + 1;
    ll potX = a[i];

    ll levelsIcanCross =
        upper_bound(all(prefStrikes), swordsIcanuse) - prefStrikes.begin() - 1;
    score = max(score, potX * levelsIcanCross);
    // cout << swordsIcanuse << " " << potX << " " << levelsIcanCross << " "
    //      << score << endl;
  }
