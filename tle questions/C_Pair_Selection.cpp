...
void solve() {
  ...
  fo(i, 0, 100) {
    ...
    vdbl trans(n);
    fo(i, 0, n) { trans[i] = inp[i].f - x * inp[i].s; }

    vdbl topk(k);
    partial_sort_copy(trans.begin(), trans.end(), topk.begin(), topk.end(),
                      std::greater<dbl>());

    dbl sum = accumulate(topk.begin(), topk.end(), 0.0);
    if (sum >= 0) {
      l = x;
    } else {
      r = x;
