...
void solve() {
  ...
  ll ud = query(lx, 2 + d - lx);
  ll dd = query(2 + d - ry, ry);

  auto ans = [](ll x, ll y) { cout << "! " << x << " " << y << endl; };

  if (query(lx + ud / 2, 2 + d - lx - ud / 2) == 0) {
    ans(lx + ud / 2, 2 + d - lx - ud / 2);
  } else {
    ans(2 + d - ry - dd / 2, ry + dd / 2);
  }
}
