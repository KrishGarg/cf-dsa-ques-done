...
void solve() {
  ...
  vvll up(n + 1, vll(LOGN, -1));
  vll depth(n + 1);
  vvll cost(n + 1, vll(LOGN, 0));

  auto dfs = [&](ll u, ll p, ll d, ll eW, auto&& f) -> void {
    depth[u] = d;
    up[u][0] = p;
    cost[u][0] = eW;
    ...
    for (auto& [v, w] : adj[u]) {
      if (v != p) f(v, u, d + 1, w, f);
    }
  };

  dfs(1, -1, 0, 0, dfs);

  auto query1 = [&](ll u, ll v) {
  ...
  auto query2 = [&](ll u, ll v, ll k) {
    ...
    ll l = depth[a] - depth[lca], r = depth[b] - depth[lca], tot = l + r + 1;
    ll riseFrom, toRise;
    if (k <= l + 1) {
      riseFrom = a;
      toRise = k - 1;
  ...
  while (true) {
    ...
    if (type == "DIST") {
      cout << query1(a, b) << endl;
    } else {
      ll k;
      cin >> k;
      cout << query2(a, b, k) << endl;
    }
  }
}
