...
void solve() {
  ...
  vll pref(n + 1, 0);
  fu(i, 1, n) { pref[i] = pref[i - 1] + b[i]; }

  vll addXtra(n + 1, 0), cnt(n + 1, 0);
  fu(i, 1, n) {
    ll j = upper_bound(all(pref), a[i] + pref[i - 1]) - pref.begin();
    cnt[i] += 1;
    if (j > n) continue;
    cnt[j] -= 1;
    addXtra[j] += a[i] + pref[i - 1] - pref[j - 1];
  }

  fu(i, 1, n) { cnt[i] += cnt[i - 1]; }

  fu(i, 1, n) { cout << (addXtra[i] + cnt[i] * b[i]) << " "; }
  cout << endl;
}
